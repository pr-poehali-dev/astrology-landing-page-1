...
const steps = [
  { num: "1", text: "Вы присылаете 3–8 дат важных событий из жизни: свадьба, рождение детей, смена работы, переезды, крупные покупки" },
  { num: "2", text: "Я анализирую карту и определяю точное время рождения с погрешностью 5–10 минут" },
  { num: "3", text: "Вы получаете уточнённое время и готовы к точному прогнозу — без ошибок в 30–50%" },
];

const benefits = [
  { icon: "ScanSearch", text: "Точность до 5–10 минут — необходимо для корректного прогноза" },
  { icon: "FileText", text: "Особенно важно для рождённых в СССР, где время фиксировалось небрежно" },
  { icon: "AlertTriangle", text: "Без ректификации прогноз может быть неточным на 30–50%" },
  { icon: "ShieldCheck", text: "Конфиденциально: ваши данные удаляются после работы" },
];

export default function LandingTime() {
  const [formData, setFormData] = useState({ name: "", phone: "+7 ", birth: "", comment: "", source: "landing-time" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.birth.trim()) return setFormError("Введите дату и место рождения");
  ...
  return (
    <div className="min-h-screen bg-[#050A14] text-white font-montserrat">
      <Helmet>
        <title>Ректификация времени рождения — уточним с точностью до 5 минут | StarsBiz</title>
        <meta name="description" content="Не знаете точное время рождения? Уточним его с точностью до 5–10 минут по ключевым событиям вашей жизни. Без ректификации прогноз ошибается на 30–50%." />
        <link rel="canonical" href="https://starsbiz.ru/time" />
      ...
      <main className="max-w-2xl mx-auto px-6 py-12 space-y-12">
        ...
        <section className="space-y-5 text-center">
          ...
          <h1 className="font-cormorant text-4xl md:text-5xl font-light leading-tight">
            Уточним время вашего рождения<br />
            <span className="text-[#D4AF37] italic">с точностью до 5 минут</span>
          </h1>
          <p className="text-white/60 text-base leading-relaxed max-w-lg mx-auto">
            Не знаете точного времени? Это не приговор.
            По ключевым событиям жизни мы восстановим его и сделаем прогноз точным.
          </p>
          <div className="flex items-center justify-center gap-2 text-[#D4AF37]/70 text-sm">
            <Icon name="Star" size={14} />
            <span>Конфиденциально · Точность до 5 мин · За 24 часа</span>
            <Icon name="Star" size={14} />
          </div>
        </section>
        ...
        <section className="space-y-4">
          <h2 className="font-cormorant text-2xl text-center text-white/80">Как это работает</h2>
          <div className="space-y-3">
            {steps.map((s) => (
              <div key={s.num} className="flex items-start gap-5 bg-[#0A1628] border border-[#D4AF37]/10 px-5 py-4">
                <span className="font-cormorant text-2xl text-[#D4AF37] flex-shrink-0 leading-none mt-0.5">{s.num}</span>
        ...
        <section className="space-y-3">
          {benefits.map((b) => (
            <div key={b.text} className="flex items-start gap-4 bg-[#0A1020] border border-[#D4AF37]/10 px-5 py-4">
        ...
        <section className="space-y-6">
          <div className="text-center space-y-1">
            <h2 className="font-cormorant text-3xl font-light">Оставить заявку</h2>
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input
                type="text"
                placeholder="Дата и место рождения (время приблизительное) *"
                value={formData.birth}
                onChange={(e) => setFormData((p) => ({ ...p, birth: e.target.value }))}
                className="w-full bg-[#0A1020] border border-white/15 text-white placeholder-white/30 px-5 py-4 text-sm focus:outline-none focus:border-[#D4AF37]/60 transition-colors"
              />
              <textarea
                placeholder="Перечислите 3–8 дат важных событий: свадьба, рождение детей, переезды, смена работы, крупные покупки..."
                rows={4}
